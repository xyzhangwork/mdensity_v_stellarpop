...
def fit_and_clean(templates, galaxy, velscale, start, goodpixels0, lam, lam_temp):
    ...
    pp = ppxf(templates, galaxy, np.full_like(galaxy,0.01), velscale, start,
...
if __name__ == '__main__':
    ...
    stacked = fits.open('stacked_spec.fits')
    ...
    for j in range(len(f_in)):
        ...
        if sigma < 400:
            ...
            velscale = C * np.diff(np.log(lam2[-2:]))[0]
            ...
            resid0 = np.nan_to_num(resamp0.outy, copy=False, nan=-999, posinf=-999, neginf=-999) - \
                     np.nan_to_num(resamp2.outy, copy=False, nan=-999, posinf=-999, neginf=-999)
