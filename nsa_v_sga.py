...
if __name__ == '__main__':
    ...
    solar_l = {'u': 6.39, 'g': 5.11, 'r': 4.65, 'i': 4.53,
               'z': 4.5}  # SDSS filters, from https://mips.as.arizona.edu/~cnaw/sun.html
    ...
    gr_colors1,gr_colors2,gr_colors_in=[],[],[]
    samp_color = [[], []]
    re_cog = []
    min_10s,min_20s,mout_20s,mout_30s=[],[],[],[]
    mout_5s=[]
    minres,mout2res=[],[]
    counting = 0
    ...
    ifus0, idx, _ = np.intersect1d(manga_sga[1].data['plateifu'],
                                            gals,
                                            return_indices=True)

    for gal in manga_sga[1].data[idx]:
        ifu = gal['plateifu']
        sid = gal['sgaid']
        ...
        r_in = hdu[1].data['psfsize_r'][0]

        dlum_r = dlum_r
        dlum_g = dlum_g
        lr_limit=np.nanmax(22.5 - 2.5 * np.log10(hdu[1].data['R_INTENS'][0]))
        if lr_limit > 28:
            xmax = np.where((22.5 - 2.5 * np.log10(hdu[1].data['R_INTENS'][0]) > 28) * 1)[0][0]
            if num>0:
                dlum_r[xmax:] = np.nan
                dlum_g[xmax:] = np.nan
        ...
        rekpc_tmp = re_tmp * (d_a * np.pi / (3600 * 180))
        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=5:
            lumout_5 = np.log10(10 ** lumtot - 10 ** (func_r((5 / (d_a * np.pi / (3600 * 180))) ** 0.25)))
        else:
            lumout_5 =np.nan

        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=10:
            lumin_10 = func_r((10 / (d_a * np.pi / (3600 * 180))) ** 0.25)
        else:
            lumin_10 =np.nan
        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=20:
            lumin_20 = func_r((20 / (d_a * np.pi / (3600 * 180))) ** 0.25)
            lumout_20 = np.log10(10 ** lumtot - 10 ** (func_r((20 / (d_a * np.pi / (3600 * 180))) ** 0.25)))
        else:
            lumin_20 =np.nan
            lumout_20=np.nan
        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=30:
            lumout_30 = np.log10(10 ** lumtot - 10 ** (func_r((30 / (d_a * np.pi / (3600 * 180))) ** 0.25)))
        else:
            lumout_30 = np.nan
        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=rekpc_tmp:
            lumin_re=func_r((rekpc_tmp / (d_a * np.pi / (3600 * 180))) ** 0.25)
        else:
            lumin_re = np.nan
        if dx_tmp[-1]* (d_a * np.pi / (3600 * 180))>=2*rekpc_tmp:
            lumout_2re=np.log10(10 ** lumtot - 10 ** (func_r((2 * rekpc_tmp/ (d_a * np.pi / (3600 * 180))) ** 0.25)))
        else:
            lumout_2re = np.nan

        gr_color = -2.5 * np.log10((10 ** func_g(xmax0 ** 0.25) - 10 ** func_g(r_in ** 0.25)) / (
                    10 ** func_r(xmax0 ** 0.25) - 10 ** func_r(r_in ** 0.25))) + (solar_l['g'] - solar_l['r'])
        ...
        if mass0<0 or mass1<0:
            print(ifu,sid,'negative mass',mass0,mass1)
            continue
        ...
        if 1.5*re_tmp<dx_tmp[-1]:
            gr_color_in = -2.5 * np.log10((10 ** func_g((1.5*re_tmp) ** 0.25) - 10 ** func_g(r_in ** 0.25)) / (
                    10 ** func_r((1.5*re_tmp) ** 0.25) - 10 ** func_r(r_in ** 0.25))) + (solar_l['g'] - solar_l['r'])
        else:
            gr_color_in=gr_color

        gr_colors_in.append(gr_color_in)
        ...
        mass_sga1.append(mass1)
        mass_sga1_2.append(mass1_2)
        minres.append(lgml0 + lumin_re)
        mout2res.append(lgml0 + lumout_2re)
        ifus.append(ifu)
        lumr.append(lumtot)
        mout_5s.append(lgml0 + lumout_5)

        min_10s.append(lgml0 + lumin_10)
        min_20s.append(lgml0 + lumin_20)
        mout_20s.append(lgml0 + lumout_20)
        mout_30s.append(lgml0 + lumout_30)

        if parameters[1].data['plateifu'].__contains__(ifu):
            ifu_cog.append(ifu)
            re_cog.append(re_tmp)
            colum_g, colum_r, colum_z = fits.Column(name='g', array=g, format='E'), \
                                        fits.Column(name='r', array=r, format='E'), \
                                        fits.Column(name='z', array=z, format='E')
            xcog = dx_tmp / re_tmp
            func_cog = interpolate.interp1d(xcog**0.25, r_tmp)
            column_x = fits.Column(name='dx(arc)', array=dx_color, format='E')
            dx_cog.append(np.linspace(xcog[0], xcog[-1], num=100))
            dy_cog.append(func_cog(np.linspace(xcog[0], xcog[-1], num=100)**0.25))
            samp_color.append(fits.BinTableHDU.from_columns([column_x, colum_g, colum_r, colum_z]))

    t = Table([ifus, mass_sga0,mass_sga0_2, lumr, mass_sga1,mass_sga1_2, gamma, re_arc,rmax_arc, re_kpc, re_nsa_arc, re_nsa_kpc,
               psf_g, psf_r, psf_z, ba,pa,gr_colors1,gr_colors_in,min_10s,min_20s,mout_20s,mout_30s,minres,mout2res,mout_5s],
              names=['plateifu', 'mass_sga_rc15', 'mass2_sga_rc15','lum_r','mass_sga_b19','mass2_sga_b19',
                     'lg_M/L', 'Re_arc','Rmax_arc', 'Re_kpc', 'Re(NSA)_arc',
                     'Re(NSA)_kpc', 'PSFSIZE_G', 'PSFSIZE_R', 'PSFSIZE_Z', 'ba',
                     'pa','gr_1psf','gr_in','Min_10kpc','Min_20kpc','Mout_20kpc','Mout_30kpc',
                     'Min_re','Mout_2re','Mout_5kpc'])
    t.write('sga_mass_new_'+names[num]+'3.fits', overwrite=True)